...
export default function FloorPage() {
  ...
  // Check if floor is like "etage-1" or just "1"
  if (/^etage-?\d+$/i.test(floor) || /^etageplan-?\d+$/i.test(floor)) {
    normalizedFloor = `Etageplan_${floor.replace(/\D/g, '')}`;
  } else {
    normalizedFloor = `Etageplan_${floor}`;
  }
